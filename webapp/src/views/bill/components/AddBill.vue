...
const emit = defineEmits(["close", "onBillAdded", "onBillUpdated"]);
...
const billAmount = ref(""); // 账单金额
const billType = reactive({
  id: 0,
  name: "",
}); // 账单类型
const remark = ref("");
...
const handelChangeDate = (date) => {
  ...
  const temp = date.selectedValues;
  const currentYear = new Date().getFullYear();
  temp.unshift(currentYear);
  selectedDate.value = new Date(temp);
};

const chooseBillType = (type) => {
  billType.id = type.id;
